...
public class EventServiceImplTest {
    ...
    public void testCreate() {
        ...
        when(eventDao.createEvent(anyString(), anyString(), any(), anyLong(), anyLong(), anyLong(), anyLong())).thenReturn(new Event.Builder()
                .eventId(ID)
                .name(NAME)
    ...
    public void testCreateAlreadyExists() {
        // 1. Precondiciones
        // Defino el comportamiento de la clase mock de UserDao
        when(eventDao.createEvent(eq(NAME),eq(DESCRIPTION),eq(DATE),eq(START_TIME_ID),eq(END_TIME_ID),eq(DURATION),eq(NEIGHBORHOOD_ID))).thenThrow(RuntimeException.class);
