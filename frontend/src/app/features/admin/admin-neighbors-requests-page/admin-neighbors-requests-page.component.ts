...
export class AdminNeighborsRequestsPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    const title = this.translate.instant(
      AppTitleKeys.ADMIN_NEIGHBORS_LIST_PAGE,
    );
  ...
  loadUsers(currentRoute: string): void {
    ...
    if (currentRoute === 'neighbors/requests') {
      const title = this.translate.instant(
        AppTitleKeys.ADMIN_NEIGHBORS_REQUESTS_PAGE,
      );
      this.titleService.setTitle(title);
    ...
    const actionDetails = this.neighbors
      ? {
          title: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.REMOVE_NEIGHBOR',
          ),
          message: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.ARE_YOU_SURE_YOU_WANT_TO_REMOVE_USERNAME_AS_A_NEIG',
            { userName: user.name },
          ),
          confirmText: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.YES_REMOVE',
          ),
          ...
          errorMessage: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.WE_ENCOUNTERED_AN_ISSUE_WHILE_TRYING_TO_REMOVE_USE',
            { userName: user.name },
          ),
        }
      : {
          title: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.REJECT_USER_REQUEST',
          ),
          message: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.ARE_YOU_SURE_YOU_WANT_TO_DECLINE_THE_REQUEST_FROM_',
            { userName: user.name },
          ),
          confirmText: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.YES_REJECT',
          ),
          successMessage: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.THE_REQUEST_FROM_USERNAME_HAS_BEEN_SUCCESSFULLY_DE',
            { userName: user.name },
          ),
          errorMessage: this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.WE_ENCOUNTERED_AN_ISSUE_WHILE_TRYING_TO_DECLINE_TH',
            { userName: user.name },
          ),
        };
    ...
    this.confirmationService
      .askForConfirmation({
        ...
        cancelText: this.translate.instant(
          'ADMIN-NEIGHBORS-REQUESTS-PAGE.CANCEL',
        ),
      })
      .subscribe(confirmed => {
        if (confirmed) {
  ...
  verifyUser(user: User) {
    this.userService.verifyUser(user).subscribe({
      ...
      error: () => {
        this.toastService.showToast(
          this.translate.instant(
            'ADMIN-NEIGHBORS-REQUESTS-PAGE.VERIFICATION_FOR_USER_USERNAME_FAILED_PLEASE_TRY_A',
            { userName: user.name },
          ),
          'error',
        );
