...
export class AffiliationService {
  ...
  public getAffiliation(url: string): Observable<Affiliation> {
    return this.http
      .get<AffiliationDto>(url)
      .pipe(
        mergeMap((affiliationDto: AffiliationDto) =>
  ...
  public getAffiliations(
    ...
    let affiliationsUrl: string = this.linkService.getLink(
      LinkKey.AFFILIATIONS,
    );

    let params = new HttpParams();

    if (queryParams.inNeighborhood)
      params = params.set('inNeighborhood', queryParams.inNeighborhood);
    if (queryParams.forWorker)
      params = params.set('forWorker', queryParams.forWorker);
  ...
  public createAffiliation(neighborhoodUrl: string): Observable<string | null> {
    let affiliationsUrl: string = this.linkService.getLink(
      LinkKey.AFFILIATIONS,
    );
    let unverifiedWorkerRoleUrl: string = this.linkService.getLink(
      LinkKey.UNVERIFIED_WORKER_ROLE,
    );
    ...
    return this.http.post(affiliationsUrl, body, { observe: 'response' }).pipe(
      ...
      catchError(error => {
        console.error(
          'Error creating Affiliation for worker',
          workerUrl,
          error,
        );
        return of(null);
   ...
   */
  public createAffiliations(
    neighborhoodUrls: string[],
    ...
    const affiliationRequests: Observable<string | null>[] =
      neighborhoodUrls.map(url => this.createAffiliation(url));

    return forkJoin(affiliationRequests);
  }

  public verifyWorker(workerUrl: string): Observable<Affiliation> {
    const affiliationsUrl: string = this.linkService.getLink(
      LinkKey.AFFILIATIONS,
    );
    const neighborhoodUrl: string = this.linkService.getLink(
      LinkKey.NEIGHBORHOOD_SELF,
    );
    const verifiedWorkerRoleUrl: string = this.linkService.getLink(
      LinkKey.VERIFIED_WORKER_ROLE,
    );
    ...
    return this.http
      .patch<AffiliationDto>(
        affiliationsUrl,
        { workerRole: verifiedWorkerRoleUrl },
        { params },
      )
      .pipe(
  ...
  public rejectWorker(workerUrl: string): Observable<Affiliation> {
    ...
    const rejectedWorkerRoleUrl: string = this.linkService.getLink(
      LinkKey.REJECTED_WORKER_ROLE,
    );
    ...
    return this.http
      .patch<AffiliationDto>(
        affiliationsUrl,
        { workerRole: rejectedWorkerRoleUrl },
        { params },
      )
      .pipe(
        mergeMap(newAffiliation => mapAffiliation(this.http, newAffiliation)),
  ...
  public deleteAffiliation(neighborhoodUrl: string): Observable<void> {
    const affiliationsUrl: string = this.linkService.getLink(
      LinkKey.AFFILIATIONS,
    );
    let workerUrl: string = this.linkService.getLink(LinkKey.USER_WORKER);
...
export function mapAffiliation(
  ...
  return forkJoin([
    http
      .get<WorkerDto>(affiliationDto._links.worker)
      .pipe(mergeMap(workerDto => mapWorker(http, workerDto))),
