...
export class ContactService {
  ...
  public getContact(url: string): Observable<Contact> {
    return this.http
      .get<ContactDto>(url)
      .pipe(map((contactDto: ContactDto) => mapContact(contactDto)));
  }
  ...
  public getContacts(
    ...
    let contactsUrl: string = this.linkService.getLink(
      LinkKey.NEIGHBORHOOD_CONTACTS,
    );
    ...
    return this.http
      .get<ContactDto[]>(contactsUrl, { params, observe: 'response' })
      .pipe(
        map(response => {
