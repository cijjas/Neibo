...
export class AttendanceService {
  ...
  public getAttendances(
    ...
    let attendanceUrl: string = this.linkService.getLink(
      LinkKey.NEIGHBORHOOD_ATTENDANCE,
    );
    ...
    if (queryParams.forEvent !== undefined)
      params = params.set('forEvent', queryParams.forEvent.toString());
    if (queryParams.forUser !== undefined)
      params = params.set('forUser', queryParams.forUser.toString());

    return this.http
      .get<AttendanceDto[]>(attendanceUrl, { params, observe: 'response' })
      .pipe(
        mergeMap(response => {
  ...
  public createAttendance(eventUrl: string): Observable<string | null> {
    let attendanceUrl: string = this.linkService.getLink(
      LinkKey.NEIGHBORHOOD_ATTENDANCE,
    );
  ...
  public deleteAttendance(forEvent: string): Observable<void> {
    let attendanceUrl: string = this.linkService.getLink(
      LinkKey.NEIGHBORHOOD_ATTENDANCE,
    );
...
export function mapAttendance(
  ...
  return forkJoin([
    http
      .get<UserDto>(attendanceDto._links.attendanceUser)
      .pipe(mergeMap(userDto => mapUser(http, userDto))),
    http
      .get<EventDto>(attendanceDto._links.event)
      .pipe(mergeMap(eventDto => mapEvent(http, eventDto))),
  ]).pipe(
    map(([user, event]) => {
      return {
        user: user,
