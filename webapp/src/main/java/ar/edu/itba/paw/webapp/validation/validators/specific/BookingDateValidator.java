...
public class BookingDateValidator implements ConstraintValidator<BookingDateConstraint, BookingDto> {
    ...
    public boolean isValid(BookingDto bookingDto, ConstraintValidatorContext context) {
        ...
        if (!availabilityOptional.isPresent()) {
            context.buildConstraintViolationWithTemplate("No availability found for the specified shift and amenity")
                    .addConstraintViolation();
            return false;
        }
        ...
        if (!reservationDayName.equalsIgnoreCase(availabilityOptional.get().getShift().getDay().getDayName())) {
